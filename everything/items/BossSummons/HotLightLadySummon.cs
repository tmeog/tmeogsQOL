...
namespace tmeogsQOL.everything.items.BossSummons
{
	public class HotLightLadySummon : ModItem
	{
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback){
            ...
            Projectile.NewProjectile(player.GetSource_ItemUse(Item), Pos.X, Pos.Y, 0, 0, ModContent.ProjectileType<NPCSpawnProj>(), 0, 0, Main.myPlayer, NPCID.HallowBoss);

            string text = "Empress of Light has awoken!";
